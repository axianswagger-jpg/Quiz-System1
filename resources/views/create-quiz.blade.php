...
<style>
  ...
  textarea.cq-input { resize: vertical; min-height: 80px; border-radius: 6px 6px 0 0; }
  ...
  .cq-question-header span {
    ...
    color: #68c3ff; letter-spacing: 0.04em;
  }
  ...
  .cq-correct-hint {
    font-size: 11px; color: #68c3ff;
    margin: 8px 0 12px; opacity: 0.7;
  }
  ...
  <form method="POST" action="{{ route('quiz.store') }}" style="margin-top:16px;">
    ...
    <div class="cq-card">
      ...
      <p style="color:#9fb0cb;font-size:13px;margin:-8px 0 16px;">
        Add at least 1 question. Select ● to mark the correct answer.
      </p>

      <div id="questionsContainer">
        @if(old('questions'))
          @foreach(old('questions') as $qIndex => $qData)
            <div class="cq-question-card" id="q-{{ $qIndex }}">
              <div class="cq-question-header">
                <span>QUESTION {{ $qIndex + 1 }}</span>
                @if($qIndex > 0)
                  <button type="button" class="btn-remove" onclick="removeQuestion({{ $qIndex }})">✕ Remove</button>
                @endif
              </div>
              <input type="text" name="questions[{{ $qIndex }}][question_text]"
                class="cq-input" placeholder="Enter your question here..."
                value="{{ $qData['question_text'] ?? '' }}" required>
              <div class="cq-correct-hint">● Select the correct answer</div>
              @foreach(['A','B','C','D'] as $oIndex => $letter)
                <div class="cq-option-row">
                  <input type="radio" name="questions[{{ $qIndex }}][correct]" value="{{ $oIndex }}"
                    {{ ($qData['correct'] ?? 0) == $oIndex ? 'checked' : '' }}>
                  <input type="text" class="cq-option-input"
                    name="questions[{{ $qIndex }}][options][]"
                    placeholder="Option {{ $letter }}"
                    value="{{ $qData['options'][$oIndex] ?? '' }}"
                    {{ $oIndex < 2 ? 'required' : '' }}>
                </div>
              @endforeach
            </div>
...
<script>
let qCount = {{ old('questions') ? count(old('questions')) : 1 }};
